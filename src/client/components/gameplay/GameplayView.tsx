  ...
  return (
    <div className="w-full h-full max-h-full bg-[#FFF8F0] dark:bg-[#0f1419] flex flex-col overflow-hidden relative font-sans text-sm text-neutral-900 dark:text-white/95">
      ...
      <div className="flex-1 flex flex-col items-center px-4 py-2 gap-2 overflow-y-auto w-full min-h-0">
        ...
        {/* Hint Button - positioned between images and feedback */}
        {!isGameOver && !isCompleted && !isCreator && onRevealHint && (
          <div className="flex flex-col items-center flex-shrink-0">
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full transition-colors ${
                hintModeActive
                  ? 'bg-game-primary dark:bg-[#f0d078] text-white dark:text-[#1a2332]'
                  : 'bg-neutral-200 dark:bg-white/10 text-neutral-700 dark:text-white/70 hover:bg-neutral-300 dark:hover:bg-white/20'
              }`}
            >
      ...
      {!isGameOver && !isCompleted && !isCreator && (
        <GuessInput onSubmit={handleAnswerSubmit} isSubmitting={isSubmitting} />
      )}
    </div>
  );
}
