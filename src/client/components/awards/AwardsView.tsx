...
export const AwardsView: Devvit.BlockComponent<AwardsViewProps> = ({
    ...
    const unlockedCount = BADGES.filter(b => b.condition(profile)).length;
    ...
    return (
        <vstack width="100%" height="100%" backgroundColor="#F6F7F8">
            {/* Header - Three-column layout */}
            <hstack width="100%" padding="small" alignment="middle" backgroundColor="white">
                {/* Left: Back button */}
                ...
                <hstack width="34%" alignment="center middle">
                    <text size="large" weight="bold" color="#1c1c1c">Awards</text>
                </hstack>

                {/* Right: Count */}
                <hstack width="33%" alignment="end middle">
                    <text size="large" color="#FF6D00">{unlockedCount}/{BADGES.length}</text>
                </hstack>
            </hstack>

            {/* Badges Grid - Reduced padding to show pagination */}
            <vstack padding="small" gap="medium" width="100%" grow>
                {Array.from({ length: Math.ceil(currentBadges.length / 2) }).map((_, rowIndex) => (
                    <hstack key={`row-${rowIndex}`} gap="medium" width="100%" height="49%">
                        {/* First Badge in Row */}
                        <BadgeItem
            ...
            {totalPages > 1 ? (
                <hstack width="100%" padding="small" gap="medium" alignment="center middle" backgroundColor="white">
                    <button
...
const BadgeItem = ({ badge, profile, isEmpty }: { badge?: Badge; profile: UserProfile; isEmpty: boolean }) => {
    ...
    return (
        ...
        >
            ...
            <text size="small" color="#878a8c" alignment="center" wrap>{badge.description}</text>
